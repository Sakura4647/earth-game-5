...
const game = {
    ...
    generatePath() {
        ...
        const yBottomLimit = h - (marginY + safeBottom); // 起點往上提
    ...
    checkCollision(x: number, y: number) {
        ...
        if (!this.ctx.isPointInStroke(this.path2D, x, y)) collided = true;
        else {
    ...
    updateProgressFromInfo(info: { collided: boolean, closestIdx: number, dist: number }) {
        ...
        if (percent >= 98) { STATE.progress = 100; this.finishGame(); }
    },
    ...
    calculateResult() {
        ...
        if (p >= 70) return { score: 3, message: '脾胃強健，動作靈巧', subMessage: '手部肌肉非常穩定，表現亮眼。' };
        else if (p >= 35) return { score: 2, message: '脾胃不錯，穩中帶進', subMessage: '穩定度不錯，下次就能走得更遠。' };
        else return { score: 1, message: '脾胃警告，需多調養', subMessage: '深呼吸，下一次會更穩的。' };
    }
};
